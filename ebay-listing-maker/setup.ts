...
async function main() {
  ...
  console.log("  Log into eBay manually (2FA, etc.)");
  ...
  await localPage.goto("https://signin.ebay.com/ws/eBayISAPI.dll?SignIn");

  await waitForEnter("Press ENTER after you've logged into eBay... ");

  const cookies = await localStagehand.context.cookies();
  const ebayCookies = cookies.filter((c) => c.domain?.includes("ebay.com"));
  console.log(`Captured ${ebayCookies.length} eBay cookies.`);

  await localPage.goto("https://www.ebay.com/", { waitUntil: "domcontentloaded", timeoutMs: 60000 });
  ...
  await bbStagehand.context.addCookies(ebayCookies);
  console.log("Cookies injected into Browserbase context.");

  const bbPage = bbStagehand.context.pages()[0];
  try {
    await bbPage.goto("https://www.ebay.com/", { waitUntil: "domcontentloaded", timeoutMs: 60000 });
  } catch {}
  ...
  const pageState = await bbStagehand.extract(
    "Is this a login/sign-in page or is the user logged into eBay? Look for a sign-in form vs the eBay homepage with the user's name visible.",
  ...
  if (pageState.isLoginPage) {
    console.log("\nCookies didn't carry over — eBay required re-login from this IP.");
    console.log("Re-run 'npm run setup' and complete any checkpoint in the local browser.");
  } else {
    console.log(`\n✅ Success! eBay is authenticated in the Browserbase context.`);
    console.log(`Context ID: ${contextId}`);
    console.log("Run 'npm start' to create an eBay listing.");
  }
}
