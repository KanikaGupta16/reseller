...
async function main() {
  ...
  console.log("  Log into Depop manually.");
  ...
  await localPage.goto("https://www.depop.com/login/");

  await waitForEnter("Press ENTER after you've logged into Depop... ");

  let depopCookies: Awaited<ReturnType<typeof localStagehand.context.cookies>> = [];
  ...
  try {
    ...
    depopCookies = cookies.filter((c) => c.domain?.includes("depop.com"));
    console.log(`Captured ${depopCookies.length} Depop cookies.`);

    // Try to grab localStorage/sessionStorage from the current page
    storage = await localPage
      .evaluate(() => {
      ...
      .catch((e) => {
        console.log("Could not read page storage, continuing with cookies only:", (e as Error).message);
    ...
    console.log("Browser session lost, continuing with captured cookies:", (e as Error).message);
  }
  ...
  await bbStagehand.context.addCookies(depopCookies);
  console.log("Cookies injected into Browserbase context.");

  const bbPage = bbStagehand.context.pages()[0];
  try {
    await bbPage.goto("https://www.depop.com/", { waitUntil: "domcontentloaded", timeoutMs: 60000 });
  } catch {}
  ...
  const pageState = await bbStagehand.extract(
    "Is this the Depop login page or is the user logged in? Look for a login form vs the Depop home feed with a profile/sell icon.",
  ...
  if (pageState.isLoginPage) {
    console.log("\nCookies didn't carry over — Depop required re-login from this IP.");
    console.log("Re-run 'npm run setup' and complete any checkpoint in the local browser.");
  } else {
    console.log(`\n✅ Success! Depop is authenticated in the Browserbase context.`);
    console.log(`Context ID: ${contextId}`);
    console.log("Run 'npm start' to create a Depop listing.");
  }
}
